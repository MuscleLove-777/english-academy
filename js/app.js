...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('english-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('english-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('english-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('english-progress', JSON.stringify(this.progress));
        localStorage.setItem('english-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        var html = '<div class="fade-in">' +
            ...
            '<h2>英語 Academy へようこそ</h2>' +
            '<p>TOEIC対策からビジネス英語まで、実践的な英語力を体系的に学べるプラットフォームです。</p>' +
            '</div>' +
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('english-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('english-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            ...
            localStorage.removeItem('english-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
